"""Physics-guided diagnostic analysis for the MCC5-THU motor dataset.

This script extracts run-level vibration and current features using
keyphase-based angle resampling and evaluates constituent-evidence preservation,
approximate additivity, and isolated-fault separability under condition-held-out
probes.

Compound labels are resolved from the recorded fault labels, and compound runs
are not used to select single-fault signature features.
"""
...
MIN_KEYPHASE_EDGE_GAP_SAMPLES = 64
EPS = 1e-12
...
def resolve_source_csv(path_text):
    ...
    if len(matches) == 1:
        return matches[0]
    raise FileNotFoundError(f'Could not resolve source CSV: {path_text}')
    ...
    for i, row in runs.iterrows():
        ...
        try:
            ...
            if arr.shape != (EXPECTED_SAMPLES, 6):
                bad.append((run_id, f'cache shape {arr.shape}'))
            else:
                _ = float(arr[-1, -1])
            del arr
        except Exception as exc:
            bad.append((run_id, f'cache error {repr(exc)}'))
        try:
            _ = resolve_source_csv(row['source_csv'])
        except Exception as exc:
            bad.append((run_id, f'csv error {repr(exc)}'))
        if (i + 1) % 50 == 0:
            print(f'{i + 1}/{len(runs)} checked')
    if bad:
        raise RuntimeError(f'Input problems found. First examples: {bad[:10]}')
    print('\nPASS: cache + original CSV paths valid.')
...
def extract_all_run_features(runs, windows):
    ...
    if feature_path.exists() and qc_path.exists():
        ...
        if set(cached['run_id']) == set(runs['run_id']):
            print('\nRUN_PHYSICS_FEATURES.csv already exists; reusing it.')
            return cached
    print('\n' + '=' * 100)
    print('EXTRACT RUN-LEVEL PHYSICS FEATURES')
    print('=' * 100)
    rows = []
    qc_rows = []
    ...
    for counter, (_, run_row) in enumerate(runs.iterrows(), start=1):
        ...
        csv_path = resolve_source_csv(run_row['source_csv'])
        cache_path = CACHE_DIR / f'{run_id}.npy'
        ...
        for _, wrow in selected_windows.iterrows():
            start = int(wrow['start_sample'])
            end = int(wrow['end_sample'])
            raw = np.asarray(cache[start:end, :], dtype=np.float32)
    ...
    print('\nPASS: RUN_PHYSICS_FEATURES.csv created.')
    return feature_df
...
def compute_univariate_signature_auc(feature_df, feature_cols):
    ...
    print('\n' + '=' * 100)
    print('TOP SINGLE-FAULT SIGNATURE FEATURES')
    print('=' * 100)
...
def condition_heldout_probe_auc(feature_df, feature_cols, health_label):
    ...
    print('\n' + '=' * 100)
    print('CONDITION-HELD-OUT RUN-LEVEL AUC')
    print('=' * 100)
...
def test_a_preservation_and_additivity(feature_df, signatures, compound_mapping, health_label):
    ...
    print('\n' + '=' * 100)
    print('CONSTITUENT EVIDENCE PRESERVATION')
    print('=' * 100)
...
def main():
    print('\n' + '=' * 100)
    print('PHYSICS-GUIDED FEATURE DIAGNOSTICS')
    print('=' * 100)
    runs, windows = load_complete_inventory()
    compound_mapping, mapping_table = resolve_compound_mapping(runs)
    validate_inputs(runs)
    feature_df = extract_all_run_features(runs, windows)
    feature_cols = get_numeric_feature_columns(feature_df)
    if len(feature_cols) < 10:
        raise RuntimeError(f'Too few usable physics features extracted: {len(feature_cols)}')
    print(f'\nUsable numeric physics features: {len(feature_cols)}')
    auc_univariate, signatures, health_label = compute_univariate_signature_auc(feature_df, feature_cols)
    auc_multivariate = condition_heldout_probe_auc(feature_df, feature_cols, health_label)
    preservation, pres_summary, additivity, add_summary = test_a_preservation_and_additivity(feature_df, signatures, compound_mapping, health_label)
    summary = save_final_summary(runs, feature_df, mapping_table, auc_multivariate, pres_summary, add_summary)
    print('\n' + '=' * 100)
    ...
    print('1. TEST_B_CONDITION_HELDOUT_AUC.csv')
    print('2. SIGNATURE_FEATURES.csv')
    print('3. TEST_A_PRESERVATION_SUMMARY.csv')
    print('4. TEST_A_ADDITIVITY_SUMMARY.csv')
    print('5. FINAL_DIAGNOSTIC_SUMMARY.json')
