"""Primary crossed constituent-level evaluation for the MCC5-THU motor dataset.

Nine independent constituent detectors are evaluated under recombination,
one-sided context zero-shot, and two-sided context zero-shot regimes, each
under seen and unseen operating conditions.

Mechanism-specific features and the L2 logistic-regression specification are
fixed before outer evaluation. Imputation, standardization, model fitting, and
constituent-specific threshold calibration use outer-training data only.
Thresholds are selected from grouped inner cross-validation by operating
condition. Outer-test observations are never used for preprocessing, fitting,
...
PHYSICS_DIR = ROOT / 'PHYSICS_DIAGNOSTICS_RESULTS'
RESULT_DIR = ROOT / 'FINAL_CROSSED_COMPOSITION_CONDITION_RESULTS'
RESULT_DIR.mkdir(parents=True, exist_ok=True)
PHYSICS_PATH = PHYSICS_DIR / 'RUN_PHYSICS_FEATURES.csv'
CURATED_DEFINITION_PATH = PHYSICS_DIR / 'CURATED_SIGNATURE_DEFINITION.csv'
...
def load_data():
    ...
    if not CURATED_DEFINITION_PATH.exists():
        raise FileNotFoundError(f'CURATED_SIGNATURE_DEFINITION.csv not found:\n{CURATED_DEFINITION_PATH}')
    manifest_frames = []
    ...
    definition = pd.read_csv(CURATED_DEFINITION_PATH)
    required_definition = {'mechanism_key', 'feature'}
    missing = sorted(required_definition - set(definition.columns))
    if missing:
        raise RuntimeError(f'CURATED_SIGNATURE_DEFINITION.csv missing columns: {missing}')
...
def build_feature_map_and_raw_matrices(data, definition):
    print('\n' + '=' * 130)
    print('LOCKED CURATED FEATURE MAP')
    print('=' * 130)
    feature_map = {}
    raw_matrices = {}
    feature_rows = []
    for component in COMPONENTS:
        ...
        features = sorted({feature for feature in requested if feature in data.columns and (not feature.endswith('__iqr')) and pd.api.types.is_numeric_dtype(data[feature])})
        if len(features) == 0:
            raise RuntimeError(f'{component}: no curated feature found.')
        ...
        print(f'{component:24s}: {len(features):3d} features')
        for feature in features:
            feature_rows.append({'component': component, 'feature': feature})
    pd.DataFrame(feature_rows).to_csv(RESULT_DIR / 'LOCKED_FEATURE_MAP.csv', index=False, encoding='utf-8-sig')
